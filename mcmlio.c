...
*/
  ...
  /*Changes made to original code */
    //Out_Ptr->xyz   = AllocMatrix(0,80,0,80);
...
void FreeData(InputStruct In_Parm, OutStruct * Out_Ptr)
{
  ...
  /*Changes made to original code*/
//  FreeMatrix(Out_Ptr->xyz, 0,80,0,80);
}
...
/***********************************************************
 *	3 numbers each line, denoting x,y and z co-ordinates.
 ****/
void Write_xyz(FILE * file,
			   OutStruct Out_Parm)
{
  short ia;
  short j;
  ...
  for(ia=0;ia<10;ia++) {
	  for(j=0;j<7;j++)
	  {
	  //fprintf(file, "%12.4E\t", Out_Parm.xyz[ia][j]);
		  fprintf(file, "Hello" );
      }
	  fprintf(file, "\n");
  }
